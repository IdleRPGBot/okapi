...
router.post('/:id', async (req, res) => {
  ...
  if (!req.body.url) {
    res.status(400).send({ err: 'You must provide a source url + make sure to set the Content-Type header to application/json' })
    return
  }
  ...
  switch (req.params.id) {
    case '1': ctx.drawImage(foreground, 0, 0); break
    case '2': ctx.drawImage(foreground2, 0, 0); break
  }
